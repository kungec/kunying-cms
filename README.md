...
**v1.0.24 (2026-09-22) 当前版本**
- 🧪 CI新增**全新安装E2E验证**:每次推送自动在独立MySQL环境中执行完整一键安装,
  验证安装成功、install目录自动删除、前台/后台/robots/sitemap全部可用
- 🛠 新增 router.php:开发/CI环境用PHP内置服务器即可完整运行站点(支持伪静态)

**v1.0.23 (2026-09-22)**
- 🏗 新增GitHub Actions CI:每次推送自动在PHP 7.4~8.3五个版本执行全量语法检查+Docker配置校验
- 📊 数据运维自动化:每日凌晨自动执行图片清理(可选)+验证码/登录失败记录/后台日志瘦身
- 📸 README新增真实界面截图
