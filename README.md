...
**v1.0.39 (2026-09-24) 当前版本**
- ⚡ **采集提速**:页内封面由逐张串行改为 curl_multi 8路并发下载,大幅缩短新片采集耗时
- 🧵 **多进程采集**:CLI支持 `--full --pages=N --threads=N`(1~8进程按接口×页分片并行),后台可设并行进程数
- 💾 **图片存储位置可配置**:本站目录(默认/upload/vod,可自定义)/ FTP / 阿里云OSS / S3兼容(MinIO/B2/COS),云盘直存外链地址,失败自动回退本地
- 🔌 后台新增「测试存储」一键探针,验证云盘配置连通性
- 🎬 **播放卡顿优化**:代理层session锁释放(分片恢复真并行)、视频分片真流式透传(边收边发/断连即停)、播放列表磁盘缓存1小时(VOD)、分片浏览器缓存24小时、播放器增加回看缓冲回收
- 🧭 **分类主/副分级**:分类可设「主分类/副分类/不在导航」,主分类显示在网站导航栏,副分类自动进入所属主分类的下拉列表(三主题导航同步支持)
- 🖼 **封面自动压缩**:采集入库时JPEG/WebP统一重编码为宽≤480px q80(实测178KB→29KB省84%),GIF/PNG保留原样;存量45664张历史封面由 `ky/cron/imgopt.php` 后台分批瘦身,静态资源浏览器缓存7天→30天

**v1.0.38 (2026-09-24)**
- 🐛 **修复人机验证面板被压扁**:登录页CSS对极验面板强制高度+overflow:hidden,点击按钮后挑战内容挤成44px条无法完成验证 — 移除压制样式(后台登录+kylite/kunpro/iqiyi三主题)
- 📐 **验证按钮宽度对齐**:极验控件自带固定宽度不随表单伸缩,holder/box/btn整链强制100%宽,与输入框完全对齐(全端)

**v1.0.37 (2026-09-24)**
- 🔐 **全主题安全审计**:三主题(kylite/kunpro/iqiyi)解密逐模板审计,修复JS输出转义、空源播放JS报错等问题
- 🛠 **付费主题修复**:kunpro/iqiyi 播放页空源守卫;iqiyi 补齐忘记密码页(此前直接500)与观看历史页样式
- 🛡 新增代码安全复查:求片管理视图转义、CSRF覆盖确认
- ⚡ 主题导航分类查询缓存化(每页减少2次查询)
- 🧹 生产环境一致性审计:补齐求片管理功能部署、PWA资源入库、清理遗留文件

**v1.0.36 (2026-09-24)**
- 🔴 **致命修复:前台 main.js 从未被加载**(仅preload未执行),AJAX交互全部失效 — 补上script引入
- 🟠 **框架层修复:jsonPost增加$_POST回退**,收藏/评论/积分解锁/充值下单参数永远为空的问题一次修复
- 🔍 搜索联想全站生效;用户中心补「账号设置」标签;求片/清空历史补CSRF
- 🎨 轮播图缩小加留边+交叉淡入翻页特效+左右箭头/圆点切换
- 📝 底部新增友情链接展示区;热门推荐修复空格与轮播重复
- 🧹 删除死文件与死代码(30KB旧版压缩残留等);dsv1旧主题名残留统一为kylite

**v1.0.35 (2026-09-23)**
- 🎬 kylite首页搜索横幅改为**全屏幻灯轮播**(背景大图+片名+立即播放)
- 🔍 搜索框移至header右上角(紧凑设计+联想下拉)
- 🎨 CSS全面打磨(卡片光晕/渐入动画/搜索栏重设计)
- 📱 PWA支持(添加到主屏幕)
- 🔍 JSON-LD视频富摘要 + 百度自动推送
- 💾 每日数据瘦身 + 磁盘看门狗

**v1.0.33 (2026-09-23)**
- 🎨 **kylite默认主题大规模视觉重设计**:全新设计系统+暗色模式+玻璃拟态导航+微交互动画
- 📱 移动端PWA支持(添加到主屏幕)+底部Tab栏
- 🔍 JSON-LD视频结构化数据+百度自动推送+RSS订阅
- 💾 磁盘深度清理(释放12GB)+采集源参数修复(CI-E2E发现)
...
- 📱 新增**PWA支持**:用户可将站点添加到手机主屏幕,以独立APP形态全屏运行
