...
<div class="wrap" style="margin-top:86px">
  ...
  <div class="<?= $sm === 'movie' ? 'mgrid' : ($sm === 'iqiyi' ? 'iqgrid' : 'grid g6') ?>" id="wfGrid">
    <?php if ($sm === 'movie'): foreach ($list as $v): $vv = $v; ?>
    ...
    <?php endforeach; elseif ($sm === 'iqiyi'): foreach ($list as $v): $vv = $v; ?>
    <a class="iqc" href="/index.php?s=/vod/detail&id=<?= (int)$v['id'] ?>">
      <div class="pic"><img src="<?= e(pic_url($v['pic'])) ?>" loading="lazy" alt="<?= e($v['name']) ?>">
        <?php if ($v['year']): ?><span class="tag"><?= e($v['year']) ?></span><?php endif; ?>
        <?php if ($v['vip']): ?><span class="vip">vip专享</span><?php endif; ?>
      </div>
      <div class="nm"><?= e($v['name']) ?></div>
      <div class="st"><?= e($v['remarks'] ?: (string)$v['area']) ?></div>
    </a>
    <?php endforeach; else: foreach ($list as $v): include theme_path('_vcard.php'); endforeach; endif; ?>
  </div>
  ...
  <script>
  window.wfConfig = {grid:'#wfGrid', next:<?= $page + 1 ?>, mode:'type', cardStyle:'<?= $sm === 'iqiyi' ? 'iq' : ($sm === 'movie' ? 'poster' : 'land') ?>', typeId:<?= (int)($type['id'] ?? 0) ?>, order:'<?= e($order) ?>', cls:<?= json_encode($class, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>, year:<?= json_encode($year, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>, area:<?= json_encode($area, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>};
  </script>
  <?php else: ?>
  <?= $pageHtml ?>
  <?php endif; ?>
